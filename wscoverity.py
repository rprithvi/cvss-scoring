# Helpers for accessing Coverity Web Services API v9
#
# To use:
...
class DefectServiceClient(WebServiceClient):
    ...
    def update_cvss(self, triageStoreName, cid, baseVector, baseScore, severity):
        """Programatically triage a defect.
        ...
        defectStateSpecDO = self.client.factory.create('defectStateSpecDataObj')
        defectStateSpecDO.defectStateAttributeValues = [
            self.create_defect_state_attribute_value('CVSS_Vector', baseVector),
            self.create_defect_state_attribute_value('CVSS_Score', baseScore),
            self.create_defect_state_attribute_value('CVSS_Severity', severity)]
            
        self.client.service.updateTriageForCIDsInTriageStore(triageStoreIdDO, mergedDefectIdDO, defectStateSpecDO)

    def update_cvss_audited(self, triageStoreName, cid, baseScore, severity):
        """Programatically triage a defect.
    ...
    def get_triage_info(self, triageStoreName, cid):
        """Get triage history for a particular CID.

        Keyword arguments:
        cid -- defect CID
        triageStoreName -- name of Coverity triage store
        
        Returns list of elements describing triage history having the structure defined by the Coverity web services API
        """
        ...
        triageInfoDOs = self.client.service.getTriageHistory(mergedDefectIdDO, triageStoreIdDO)
        return triageInfoDOs
    ...
    def get_merged_defects_for_stream(self, streamName):
        ...
        #filterSpecDO.ownerNameList = 'admin'
